...
public class criarEleicaoNucleo extends ActionSupport implements SessionAware {
	...
	public String execute() throws Exception {
		...
		if(bean != null){
			// é admin
			if(nomeDep.trim().equals("") || idEleicao.trim().equals("") || titulo.trim().equals("") || descricao.trim().equals("") || dataI.trim().equals("") || dataI.trim().equals("")){
				addActionError("Campos ivalidos. Nao podem ser deixados em branco/apenas espacos");
				return INPUT;
			}else{
			
				try {
					String msg = bean.criaEleicao(1, this.idEleicao, this.titulo, this.descricao, this.dataI, this.dataF, this.nomeDep);
