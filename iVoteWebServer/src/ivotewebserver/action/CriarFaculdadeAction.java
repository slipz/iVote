...
public class CriarFaculdadeAction extends ActionSupport implements SessionAware {
	...
	public String execute() throws Exception {
		...
		if(bean != null){
			// é admin
			if(nome.trim().equals("") || morada.trim().equals("") || ntlf.trim().equals("") || email.trim().equals("")){
				addActionError("Campos ivalidos. Nao podem ser deixados em branco/apenas espacos");
				return INPUT;
			}else{
				try {
	
					String msg = bean.inserirFac(this.nome, this.morada, this.ntlf, this.email);
